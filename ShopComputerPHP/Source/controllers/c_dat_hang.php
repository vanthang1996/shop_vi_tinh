...
class C_dat_hang
{
	...
	public function Xu_ly_dat_hang()
	{
		...
		$so_hoa_don=$m_dat_hang->Them_hoa_don($ma_khach_hang,date("Y-m-d"),$tong_tien,0,0,$hinh_thuc_thanh_toan,"",0);
		if($so_hoa_don)
		{
			...
			foreach($gio_hang as $item)
			{
				...
				$m_dat_hang->Them_chi_tiet_hoa_don($so_hoa_don,$ma_san_pham,$so_luong,$don_gia);	
			}
		...
		$view = "views/dat_hang/v_giao_hang.tpl";
		
			include("controllers/c_nav.php");
		$smarty->assign("navs",$navs);
		$smarty->assign("subs",$subs);
		$smarty->assign("view",$view);
		$smarty->assign("khach_hang",$khach_hang);	
			$smarty->assign("hinh_thuc_thanh_toan",$hinh_thuc_thanh_toan);
			$smarty->assign("tong_tien",$tong_tien);
			$smarty->assign("gio_hang",$gio_hang);
		$smarty->display("dat_hang/layout.tpl");
	}
	...
	public function Hien_thi_dat_hang()
	{
		...
		$smarty=new Smarty_vi_tinh();
		$smarty->assign("title","Vi tính online");
		$view = "views/dat_hang/v_dat_hang.tpl";
