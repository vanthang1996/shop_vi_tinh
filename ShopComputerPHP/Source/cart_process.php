...
if(isset($_POST["product_code"]))
{
	...
	$row=$m_products->doc_san_pham_theo_ma_san_pham($new_product['product_code']);
	...
	if(isset($_SESSION["products"])){  //if session var already exist
		...
		{
			//$_SESSION["products"][$new_product['product_code']]); //unset old item
			$new_product["product_qty"] = $new_product["product_qty"]+$_SESSION["products"][$new_product['product_code']]["product_qty"];
		}			
	}
	$_SESSION["products"][$new_product['product_code']] = $new_product;
//	print_r($new_product);
		//update products with new item array
 	$total_items =0; //count total items
 	foreach($_SESSION["products"]  as $pr){
 		$total_items+=  $pr["product_qty"];
 	}
...
if(isset($_POST["load_cart"]) && $_POST["load_cart"]==1)
{
	...
	if(isset($_SESSION["products"]) && count($_SESSION["products"])>0){ //if we have session variable
		...
		$cart_box .= "</ul>";
		$cart_box .= '<div class="cart-products-total">Tổng cộng : '.number_format($total,0,",",".").$currency.' <u><a href="login.php?dat_hang=true" title="Xem giỏ hàng">Đặt hàng</a></u></div>';
