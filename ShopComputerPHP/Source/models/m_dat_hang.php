...
class M_dat_hang extends database
{
	...
	function Them_hoa_don($ma_khach_hang,$ngay_dat,$tong_tien,$tien_dat_coc,$con_lai,$hinh_thuc_thanh_toan,$ghi_chu,$tinh_trang)
	{
		...
		$this->setQuery($sql);
		$param=array(NULL,$ngay_dat,$ma_khach_hang,$tong_tien,$tien_dat_coc,$con_lai,$hinh_thuc_thanh_toan,$ghi_chu,$tinh_trang);
		$this->execute($param);
		return  $this->getLastId();
	}
	// ma_hoa_don, ma_mon, so_luong, don_gia, mon_thuc_don
	function Them_chi_tiet_hoa_don($so_hoa_don,$ma_san_pham,$so_luong,$don_gia)
	{
		$sql="insert into ct_hoa_don values(?,?,?,?,?)";
		$this->setQuery($sql);
		$param=array($so_hoa_don,$ma_san_pham,$so_luong,$don_gia,NULL);
